...
using CSV, DataFrames, CairoMakie, Statistics, Random, ColorSchemes

# ╔═╡ ae5fdbf4-0c2d-11eb-32b8-15c794321a8c
md"
in [this funny video](https://fivethirtyeight.com/features/not-even-scientists-can-easily-explain-p-values/), scientists are asked to explain the p-value in one sentence.

!!! warning \"challenge!\"
	after completing this notebook, explain what a p-value is to a friend or colleague!
"
...
!!! example \"problem set up: A/B test\"
	we wish to design a controlled study to answer the question: does treating cauliflower plants with fertilizer affect the mass of their heads?

	consider a large population of cauliflower (\"cauli\") plants on a farm, all planted at the same time. we randomly select a set of cauli plants on the farm to \"enroll\" in our A/B test.

	in a controlled study, we randomly allocate each cauli plant to one of two groups: (A) the _control group_ or (B) the _treatment group_.
	  * cauli plants in the treatment group are fertilized weekly.
	  * cauli plants in the control group are never fertilized.

	at harvest, we measure the mass (in grams) of each cauliflower head in the two groups. 

	the _goal_ of this A/B test is to determine if the distribution of masses of cauliflower heads in the treatment group differs from the distribution of masses in the control group.
...
md"
!!! note \"remark on causality\"
	in this _controlled study_, the cauli's were randomly assigned to the control and treatment groups. consequently, we can attribute a difference in the distribution of masses between the two groups as evidence that the fertilizer *caused* the difference.

	in contrast, if this were instead _an observational study_, the decisions about which cauli's to treat with fertilizer are not random. as a consequence of this selection bias, a difference in the distribution of masses among fertilized versus unfertilized cauli's in an observational study could be due to confounding variables instead of the variable of fertilizer treatment. e.g. farmers who chose to use fertilizer could have been compensating for poor-quality soil.

	for more on causality, see [here](https://inferentialthinking.com/chapters/02/causality-and-experiments.html) and [here](https://inferentialthinking.com/chapters/02/3/establishing-causality.html).
"
...
each row corresponds to a cauli plant. 

the `group` column indicates whether the cauli was in the control or treatment group, and the `mass` column indicates its mass in grams on harvest day.
"

# ╔═╡ f531ad34-0c30-11eb-0a02-8387fae3bfe9
df = CSV.File(joinpath("..", "..", "data", "cauliflower.csv")) |> DataFrame
...
combine(groupby(df, :group), nrow => :nb_plants)
...
begin
	...
	axs = Axis(fig[1, 1], 
			   xlabel="group",
		       ylabel="head mass [g]",
		       title="cauli A/B test",
			   xticks=(1:2, [df_group[1, :group] for df_group in groupby(df, :group)])
	)
	for (i, df_group) in enumerate(groupby(df, :group))
		group = df_group[1, :group]
		boxplot!(i * ones(nrow(df_group)), df_group[:, :mass])
	end
	fig
end

# ╔═╡ 9d7f702d-0998-4769-8b73-44aca6202ec7
md"
conclusion from box plot: the mass of a cauli head in the fertilizer group tends (as measured by the median in the box plot) to be greater than those in the control group.
"

# ╔═╡ 7ba24210-0c32-11eb-1f0e-5fef1ea16ade
md"
🐸 compute the `mean` mass of cauli plants within each group.
...
combine(groupby(df, :group), :mass => mean => :mean_mass)

# ╔═╡ bafffaf0-0c33-11eb-1aca-8f40e2f26125
md"
!!! warning \"don't jump to conclusions yet!\"
	the difference in means between the control and treatment groups _could_ be due to chance, owing to the random process of allocating the cauliflowers to the control and treatment groups!

	we only have 20 cauli in each group, a tiny subset of the entire population of cauli's on the farm. 

	by chance, in our random allocation of cauli's to the control and treatment groups, we could have happened to select cauli's that e.g. harbor extra growth genes for the treatment group. or maybe a few cauli's in the control group happened to be attacked by pests during the experiment. many factors besides the fertilizer treatment could have caused this difference in means among the groups.

❓❓❓ how confident are we that the difference in the distribution of masses between the control and treatment groups is not due to chance?
"
...
**null hypothesis**: in the population of cauli plants, the distribution of masses of cauli heads grown with fertilizer treatment is the same as the distribution for caulis that do not receive fertilizer. the difference in the distributions we see above is due to chance.

**alternative hypothesis**: in the population of cauli plants, fertilized cauli heads tend to be heavier than unfertilized cauli heads. the difference in the means (a measure of central tendency) we see above is not due to chance.

✏ we now design a **test statistic**, which (i) maps our data set to a single number, (ii) expresses the difference in outcome between the A and B groups, and (iii) would differ under the null and alternative hypothesis. 

e.g. *test statistic* := (mean mass of cauli heads in the fertilizer treatment group) - (mean mass of cauli heads in the control group)

!!! note \"the test statistic is a random variable.\"
    if we could repeat the cauli A/B test over and over, there would be different allocations of the cauli plants (which harbor variation in genes, soil they are planted in, proximity to a bug nest, etc.) to the control and treatment groups. consequently, the difference in the mean masses between the control and treatment group would vary from experiment to experiment. 

	in general, the test statistic expressing the difference between the distribution of some outcome between the two groups varies from experiment to experiment because of the randomness of allocating the samples to the A and B groups.

!!! example \"a permutation test\"
	a permutation test is a Monte Carlo simulation to answer the question: 
	> if we were to repeat the A/B test over and over, what is the probability that we would observe a test statistic (which expresses the difference between the A and B group) equal to or more extreme than we did, given that the null hypothesis is true? 
	in the permuation tests, we:
	* compute the test statistic expressing the difference between the A and B group _for our observed sample of data_. conceptually, this observed test statistic is one of many possible values we could see if we were to repeat the same experiment over and over, each time randomly allocating subjects (cauli's here) to the A or B group (controlor treatment groups here).
	* simulate the repetition of this experiment by randomly re-allocating subjects (cauli's here) to the A and B groups (control and treatment group here), _assuming that the null hypothesis is true_, to obtain the distribution of the test statistic _under the null hypothesis_. the \"under the null hypothesis\" is important because we know the outcome if we switched which group any given subject was allocated to: the outcome would be the same! if we did not operate under the assumption \"the null hypothesis is true\", we would be stuck because we could not simulate the experiment.
	* from this distribution of the test statistic under the null hypothesis, compute the probability that we would observe a test statistic equal to or more extreme than the test statistic we actually observed if the null hypothesis were true. this is the **p-value**.

so if the *p-value* is 0.05, this tells us, if we could repeat this experiment many times, each time randomly allocating cauli's to the control and treatment groups, *and* if fertilizer treatment has *no* effect whatsover on the mass of the cauliflower, then in 5% of the experiments, we would see the difference in means of the masses as large as it is or larger.

so if the *p-value* is small, that means that it is unlikely that the difference in means we see is due only to the random allocation of the cauli's to the two groups. thus it must be likely that the difference in means is due to a different underlying distribution of masses among cauliflowers treated with fertilizier and those that are not.
...
see [this interactive visualization of a permutation test on alpacas](https://www.jwilber.me/permutationtest/).
...
function μ_mass(df_cauli::DataFrame, group_col_name::Symbol, which_group::String)
	df_cauli_this_group = filter(row -> row[group_col_name] == which_group, df_cauli)
	return mean(df_cauli_this_group[:, :mass])
end

# ╔═╡ 55af6ea2-0c37-11eb-040d-2fe6713d8c4b
md"
test this function by ensuring it gives the correct masses for the two groups in this experiment.
"
...
md"
🐸 write a function `μ_treat_minus_μ_control` that takes in as arguments (input):
* `df_cauli::DataFrame`: the cauli `DataFrame` from the experiment
* `group_col_name::Symbol`: the name of the column in `df_cauli` that contains the group names for `filter`ing (we will need capability this later)
and returns (outputs) the test statistic.
...
🐸 simulate the repeated random experiment under the null hypothesis by making a new column `:shuffled_group` in your cauli data frame that denotes the group to which the cauliflower plant (with the same mass) is allocated in the _simulated_ repetition of the experiment under the null hypothesis.
...
!!! hint
    overwrite the `:shuffled_group` column in your cauli data frame inside a loop over simulations.
"
...
begin
	fig2 = Figure()
	ax2 = Axis(fig2[1, 1],
			   xlabel="⟨mass⟩(treatment) - ⟨mass⟩(control) [g]",
			   ylabel="# sims",
		       title="cauli permutation test"
	)
	hist!(test_stats, color=ColorSchemes.mint[1])
	vlines!(ax2, [actual_test_stat], linestyle=:dash, color=ColorSchemes.mint[end])
	fig2
end
...
!!! hint
    no! under the null hypothesis, there is no difference between the masses of cauli's treated with fertilizier and not treated with fertilizer.
...
the Twitter thread below is a great discussion on the distinction between statistical significance and practical significance.
"
