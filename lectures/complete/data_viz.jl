...
we will use:
* `Makie.jl` for data viz. see the [Makie.jl docs](https://makie.juliaplots.org/dev/).
  * a gallery of plots [here](https://lazarusa.github.io/BeautifulMakie/) and [here](https://makie.juliaplots.org/dev/examples/plotting_functions/)
* `ColorSchemes.jl` to choose effective color schemes. see the [ColorSchemes.jl docs](https://juliagraphics.github.io/ColorSchemes.jl/stable/).
* `CSV.jl` to read in comma-separated-value `.csv` files of tabular data as `DataFrames`. see the [CSV.jl](https://csv.juliadata.org/stable/reading.html#CSV.read) docs.
* `DataFrames.jl` to work with and manipulate data tables (more later). see the [DataFrames.jl docs](https://dataframes.juliadata.org/stable/).

let's first set the `Makie.jl` theme. [here](https://makie.juliaplots.org/dev/documentation/theming/) and a list of pre-defined themes [here](https://makie.juliaplots.org/dev/documentation/theming/predefined_themes/).
"
...
the raw data is in `data/Xe_in_SBMOF-1.csv`.

the [Langmuir adsorption model](https://en.wikipedia.org/wiki/Langmuir_adsorption_model) for gas adsorption in SBMOF-1:
```math
n(p) = M\frac{Kp}{1+Kp}
``` with:
...
# ╔═╡ 61388be1-096f-4b8e-88fe-02e35921bb61
# present working directory
pwd()

# ╔═╡ f6b5719a-5382-4e41-b4a0-23a5a58a7c4e
# path to file
path_to_file = joinpath("..", "..", "data", "Xe_in_SBMOF-1.csv")

# ╔═╡ 5ce0008e-a708-49d6-affc-e11606907cc2
# read in file as data frame
data = CSV.read(path_to_file, DataFrame, comment="#")
...
begin
	...
	lines!(p, n.(p), 
		   ...
		   linewidth=4)
	scatter!(data[:, :p_bar], data[!, :n_mmol_g] , 
		color=ColorSchemes.okabe_ito[2], markersize=12, label="data", marker=:rect)
	axislegend(position=:rb)
	current_figure()
end

# ╔═╡ 8be0ae67-2f7d-435d-a2b7-0722a7d6e28e
md"*understanding the Langmuir model*

how does $K$ affect the adsorption isotherm?

🐸 draw a series of Langmuir curves with different $K$. color according to $K$.
"

# ╔═╡ d8f93cf5-064e-493e-ad10-5457c8c861a4
Ks = range(1.0, 50.0, length=8)
