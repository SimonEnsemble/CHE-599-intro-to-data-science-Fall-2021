...
# ╔═╡ fad4e5da-3815-11ec-3bea-1d7b1856a595
using DifferentialEquations, Optim, CSV, DataFrames, CairoMakie, ColorSchemes, Colors, Statistics

# ╔═╡ 90d73cdd-b43f-475a-b495-438a713eefd0
set_theme!(
    Theme(
...
# ╔═╡ 13c1f721-2916-4106-b93d-a802afc21263
md"
💡 thermistors (see [here](https://en.wikipedia.org/wiki/Thermistor)) are used as temperature sensors. because the thermistor has a nonzero thermal capacity, it takes a certain amount of time for the thermistor to thermally equilibrate with its surroundings. here, we aim to determine the time constant $\tau$ of a thermistor from (i) a mathematical model for its temperature $T(t)$ and (ii) [real, from an Arduino!] time series data $\{(t_i,T_i)\}$.

## experimental set up

![a thermistor](https://www.avx.com/en/wp-content/uploads/product-images/Thermistor-Disc-Leaded.jpg)

a student, Grant, grabbed and held a thermistor for a long time; the temperature of the thermistor eventually reached thermal equilibrium with Grant's hand and adopted his body temperature. let $T_h$ be Grant's body temperature (\"h\" for human).

then, at time $t=0$, Grant let go of the thermistor to expose its outer surface to the air, which is at ambient temperature, $T_a$.

owing to the temperature differential between the (hotter) thermistor and (colder) air, heat was transferred from the thermistor to the air. as a consequence, the temperature of the thermistor decreased and approached ambient temperature.

## dynamic model for the cooling of a thermistor

let $T=T(t)$ be the temperature of the thermistor as a function of time, $t$. assume the following ODE model governs $T(t)$. the only parameter here is the time constant $\tau$ of the thermistor, which we seek to identify from data.
...
the goal is to identify the time constant $\tau$ of the data from time series data $\{(t_i, T_i)\}_{i=1}^N$.

we will do so by posing a loss function $\ell(\tau)$ which describes the poorness of the fit of the model to the data:
```math
\begin{equation}
\ell(\tau) = \displaystyle\sum_{i=1}^N [T_i - T(t_i;\tau)]^2
\end{equation}
```
the loss penalizes deviations of the model from the data points.

we wish to tune $\tau$ so as to minimize the loss $\ell$. the optimal $\tau$ is:

```math
\begin{equation}
\tau_{opt}:= \arg\min_\tau \ell(\tau)
\end{equation}
```

### read in and viz the time series data

the data is $\{(t_i, T_i)\}_{i=1}^N$, where data point $(t_i, T_i)$ represents $T_i\approx T(t_i)$.
"

# ╔═╡ aa5af36a-843a-4f6d-98e6-d4b5cc4d4e61
data = CSV.read("ambient_heat_transfer.csv", DataFrame)

# ╔═╡ 4e3a2167-6078-4744-a3f1-4f1c665c3540
begin
	fig = Figure()
	ax  = Axis(fig[1, 1], xlabel="time, [s]", ylabel="temperature [°C]")
	scatter!(data[:, :time], data[:, :temperature], 
		     markersize=5, color=:white, strokewidth=1)
	fig
end

# ╔═╡ a24dc0de-851c-4b3b-9145-9e1e53b8eccd
md"
calculate the temperature of Grant's finger, $T_h$, by taking the average (`mean`) of the temperature reading when $t<0$. during this time, Grant was still holding onto the thermistor. so, we are assuming the thermistor was in thermal equilibrium with Grant's hand.
"

# ╔═╡ d6902bd6-ab00-4c81-9f0c-d7b90a4677c8
Tₕ = mean(filter(row -> row.time < 0.0, data)[:, :temperature])

# ╔═╡ c458c8d1-a741-461f-a1c1-63352879c92e
md"
calculate the ambient temperature $T_a$ by taking the average of the temperature for $t>40$ where we assume the thermistor has reached thermal equilibrium with the ambient air.
"

# ╔═╡ 9738892e-2576-4df0-9b4a-d7075cb39baa
Tₐ = mean(filter(row -> row.time > 40.0, data)[:, :temperature])

# ╔═╡ 922c880d-2384-418e-b209-511fbec818db
md"check that $T_h$ and $T_a$ make sense by plotting them with the data."

# ╔═╡ 0ac74f4a-ca68-4085-a824-592e4e521eee
begin
	fig2 = Figure()
	ax2  = Axis(fig2[1, 1], xlabel="time, [s]", ylabel="temperature [°C]")
	scatter!(data[:, :time], data[:, :temperature], 
		     markersize=5, color=:white, strokewidth=1, label="{(tᵢ, Tᵢ)}")
	hlines!(ax2, [Tₕ], label="Tₕ", linestyle=:dash)
	hlines!(ax2, [Tₐ], label="Tₐ", linestyle=:dash)
	axislegend()
	fig2
end

# ╔═╡ b072995c-a62b-4e17-a604-caee6b8b4e39
md"use `DifferentialEquations.jl` to set up the ODE solver for different $\tau$. see [example in docs](https://diffeq.sciml.ai/stable/tutorials/ode_example/)."

# ╔═╡ 46d75a7b-df5a-49df-8ec3-c07ecd93052a
begin
	# define ODE. so dT/dt = f(T, τ, t)
	function f(T, τ, t) 
		# return right-hand side of ODE for given (T, τ, t).
		return (Tₐ- T) / τ
	end
	
	# time span over which we wish to solve the ODE
	tspan = (0.0, 50.0)
	
	function solve_ode(τ)
		# define ODE problem
		prob = ODEProblem(f, Tₕ, tspan, τ)
		# solve ODE
		sol = solve(prob)
		return sol
	end
...
# ╔═╡ fc4c12ec-a0d4-4c92-85e0-00c8ac11f5d1
md"solve the ODE for a guess of $\tau=15$ and plot the solution on top of the data."

# ╔═╡ ee99ad6e-9fe1-4767-851d-70da3cdb33dd
T = solve_ode(15.0)

# ╔═╡ 2b7d7c93-8f65-44cf-993d-f903f163b241
T(3.0) # solution at time 3.0

# ╔═╡ 0c4b21de-7556-4a77-b9bb-78a0df441c9d
t = range(0.0, tspan[2], length=150)

# ╔═╡ 25c82d48-11d3-4b11-84e1-5e6a19e93896
begin
	fig3 = Figure()
	ax3  = Axis(fig3[1, 1], xlabel="time, [s]", ylabel="temperature [°C]")
	scatter!(data[:, :time], data[:, :temperature], 
		     markersize=5, color=:white, strokewidth=1, label="{(tᵢ, Tᵢ)}")
	lines!(t, T.(t), label="model T(t)")
	axislegend()
	fig3
end

# ╔═╡ 779eb4f3-6c32-4757-a672-750c81e13a18
md"find $\tau_{opt}$---the time constant $\tau$ that optimally fits the data---by minimizing the loss. visualize the loss function. visualize the fit of the optimal model to the data."

# ╔═╡ 4192fb91-27dc-4a77-8255-a82d996791d0
function loss(τ)
	# solve ODE for this τ
	T = solve_ode(τ)
    ...
    for i = 1:nrow(data)
        # data point (tᵢ, Tᵢ)
        tᵢ = data[i, :time]
        Tᵢ = data[i, :temperature]
        # model temp at tᵢ
        T̂ᵢ = T(tᵢ)
        # add residual to the loss
		if tᵢ > 0.0
        	ℓ += (Tᵢ - T̂ᵢ) ^ 2
		end
    end
...
# ╔═╡ 7d76d2a4-9b79-480f-87e1-a9a508e0e9ea
begin
	τs = range(0.0, 50.0, length=40)
	
	fig4 = Figure()
	ax4  = Axis(fig4[1, 1], xlabel="τ [s]", ylabel="loss")
	lines!(τs, loss.(τs))
	fig4
end

# ╔═╡ c0796aae-c538-4767-a5ef-bcedf3c2a1be
begin
	opt_res = optimize(loss, 0.0, 50.0)
	
	τ_opt = opt_res.minimizer
end

# ╔═╡ 97b08781-8474-45bd-b524-273f9a7e624f
T_opt = solve_ode(τ_opt)

# ╔═╡ a14b8323-bbe1-4b16-abb5-a73e3f680b72
begin
	fig5 = Figure()
	ax5  = Axis(fig5[1, 1], xlabel="time, [s]", ylabel="temperature [°C]")
	scatter!(data[:, :time], data[:, :temperature], 
		     markersize=5, color=:white, strokewidth=1, label="{(tᵢ, Tᵢ)}")
	lines!(t, T_opt.(t), label="fit model T(t)")
	axislegend()
	fig5
end
