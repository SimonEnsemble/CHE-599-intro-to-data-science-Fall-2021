...
using DataFrames, CSV, CairoMakie, ColorSchemes, Random
...
# ╔═╡ e97f59fb-8a70-4242-a89a-91a28289b982
...
the Cavendish [banana is threatened](https://www.theguardian.com/science/2018/aug/05/science-search-for-a-super-banana-panama-disease-gm-gene-editing) by Panama disease, caused by a fungus (if interested, watch [this video](https://www.youtube.com/watch?v=YkI3zkQ4WBo&t)). 
researchers wish to genetically modify the Cavendish banana plant to make it resistant to Panama disease.
"
...
🔨 **an A/B test to determine if gene editing provides resistance to Panama disease**

suppose we identified a gene in the banana plant, gene X, that might be associated with resistance to fungal infection. using modern gene editing tools, we can make precise edits to the DNA sequence of gene X that appears in the wild. the hope is that this mutated version of gene X will confer protection to the banana plant against fungal infection. 

here, we performed a randomized, controlled experiment to test if the mutated gene X confers greater resistance to Panama disease.

we first randomly select 60 banana plants and randomly allocate each to one of two groups:
* control group: banana plants we leave unmodified, harboring the wild gene X.
* treatment group: banana plants we modify, to harbor the mutated gene X.
none of these plants have been exposed to the fungus that causes Panama disease.

then, we expose each plant in the experiment to the fungus that causes Panama disease. the plants are all contained in a single greenhouse. 
...
md"
🍌 use the `combine` function to (i) group the banana plants in `df` by genetic variant and (ii) create a new `DataFrame`, `df_outcome`, with a new column, named `p_infected`, that contains the proportion of plants of that variant that were infected by the fungus.
...
!!! hint
	the `sum` function adds up the number of `true`'s in an array. 

	i.e. `sum([true, false, false, true, true]) == 5`
"

# ╔═╡ 2b4e1258-0e3a-11eb-3d68-554a7486c259


# ╔═╡ 41ddc6f3-7980-4731-ab17-71eb39852d24


# ╔═╡ b13afc0e-cffa-4ffa-a74e-0862d6d53861


# ╔═╡ 4814fe24-0e3a-11eb-0ce5-8793afbab239
md"
🍌 to visualize the outcome of the A/B test, use your data frame `df_outcome` to draw a bar plot where:
* thare are two bars, one for modified banana plants and one for unmodified banana plants.
* the height of each bar represents the respective proportion of banana plants of that genetic variant that were infected by the fungus. 
* label your x-axis, y-axis, and x-ticks, and make the two bars two different colors colors.
...
md"
🍌 which genetic variant (wild vs. modified) has the smallest proportion of infected plants? 

then, can we conclude that this genetic variant is _certainly_ more resistant to Panama disease than the other? why or why not?
"
...
md"🍌 to set up a permutation test, formulate a null hypothesis and an alternative hypothesis that pertains to this experiment. state it below.
...
md"🍌 write a function `proportion_infected` that takes in three arguments (inputs):
* `df_banana::DataFrame`: your banana data frame `df` from above will be passed in as this argument. each row represents a 🍌 plant. has a column of `Bool`s indicating whether the plant was genetically modified or not. also has a column of `Bool`s indicating whether or not the plant was infected. 
* `modification_col::Symbol`: the name of the column in `df_banana` that indicates whether (`true`) or not (`false`) the banana plant was genetically modified. so `df_banana[2, modification_col]` indicates whether banana plant 2 was genetically modified.
* `modified::Bool`: either `true` or `false` depending on if we want to focus on modified or unmodified bananas, respectively.
and returns (outputs) the proportion of modified (`modified == true`) or unmodified (`modified == false`) banana plants that were infected by the fungus.
"
...
md"🍌 test your function `proportion_infected` on the two different groups of banana plants. it should give the same output as `df_outcome` above."
...
* `df_banana::DataFrame` same as above
* `modification_col::Symbol` same as above
...
md"🍌 imagine we are at the beginning of the experiment where we randomly select banana plants and allocate them to one of two groups:
* group A: to be genetically modified
* group B: to be left unmodified

under the null hypothesis, the outcome of whether or not any given banana plant was infected would be the same regardless of the group to which the banana plant was allocated. thus, we can simulate a repitition of this random sampling and allocation by randomly permuting the labels in the `:modified` column of `df`.

simulate one repetition of the banana study, _operating under the assumption that the null hypothesis is true_, by permuting the `:modified` column, which effectively simulates the random allocation of healthy banana plants to the two groups. assign the permuted labels to be a new column in `df` called `:shuffled_modified`.
"
...
❗ more, statistical significance does not imply the effect is large!
