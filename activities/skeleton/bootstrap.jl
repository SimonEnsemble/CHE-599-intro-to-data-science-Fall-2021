...
using CSV, DataFrames, CairoMakie, Statistics, StatsBase, ColorSchemes
...
set_theme!(theme_black()); update_theme!(fontsize=20)
...
the data table in `mushrooms.csv` [here](https://github.com/SimonEnsemble/CHE-599-intro-to-data-science-Fall-2021/blob/master/data/mushrooms.csv) pertains to a random sample of mushrooms. 
* each row represents a mushroom that was sampled. 
the three columns are the attributes of the mushrooms:
...
md"
🍄 remove all mushrooms from the data frame, except those that (i) are poisonous _and_ (ii) are from the woods. [the remainder of this assignment will focus on poisonous mushrooms in the woods.]
"
...
md"
🍄 write a function `proportion_odor(df::DataFrame, odor::String)` that returns the proportion of mushrooms in data frame `df` that have odor `odor`. it should work as follows:

```
# calculate proportion of mushrooms that are musty
...
md"
🍄 test your function by taking a bootstrap sample of the mushrooms and computing the proportion among that bootstrap sample that are musty in odor. the proportion of musty odors in your bootstrap sample (i) should differ from the true proportion in the sample, which you computed above, and (ii) should change each time you re-run the cell doing the bootstrap sampling, since you would then be selecting a different bootstrap sample. 
"
...
md"
🍄 use 10,000 bootstrap samples to obtain the bootstrap empirical distribution of the proportion of poisonous mushrooms in the woods that are of odor creosote=c, fishy=y, foul=f, musty=m, none=n, or spicy=s.

draw a box plot with six boxes, one for each odor, depicting these bootstrap empirical distributions of the proportion of mushrooms with that odor. use the full names of the odors on the x-tick labels.

!!! hint
    a double, nested `for` loop, one for bootstrap samples, one for odors.
"
...
present your results in a data visualization:
* a bar plot with six bars, where each bar is the proportion of mushrooms of a given odor
* include [error bars](https://makie.juliaplots.org/v0.15.2/examples/plotting_functions/errorbars/) on each bar that indicate the bootstrap confidence interval.  
* use the full names of the odors on the x-tick labels. 
* make each bar a different color using `ColorSchemes` (e.g. see color brewer colors [here](https://juliagraphics.github.io/ColorSchemes.jl/stable/catalogue/)).

this is your final product!

as opposed to smelling _all_ of the poisonous mushrooms in the woods, you've smelled a random sample and made a conclusion about the entire population while quantifying your uncertainty with a bootstrap confidence interval. the uncertainy fundamentally emanates from the stochastic process of the random sampling of mushrooms from the population, which you've mimicked through bootstrap resampling. 😄
"
...
# ╔═╡ 45248a88-b35b-4107-a7ae-492106897a2a
md"
🍄 since someone said they enjoyed the documentary I recommended on Panama disease in bananas, I also recommend the documentary \"Fantastic Fungi\" on Netflix. 👀
"
